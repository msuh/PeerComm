    ...
    try {
      ...
      if (['LEAVE', 'CANDIDATE', 'OFFER', 'ANSWER'].indexOf(message.type) !== -1) {
        ...
        if(!self.urlToPeers[message.url] || self.urlToPeers[message.url].length == 0){
          console.log("first connection to url:"+message.url);
            self.urlToPeers[message.url] = [message.id];
            self.urlToLeader[message.url] = message.id;

            self.assignLeader(message.id);
            // var serverMessage = self.formServerMessage('LEADER',message.id);
